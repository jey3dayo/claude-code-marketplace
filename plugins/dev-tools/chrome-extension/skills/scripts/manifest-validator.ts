...
class ManifestValidator {
  ...
  private addError(severity: ValidationError['severity'], message: string, field?: string): void {
  ...
  private validateRequiredFields(): void {
    ...
    if (this.manifest.name && this.manifest.name.length > 75) {
      this.addError('error', 'Name must be 75 characters or less', 'name')
    }
  ...
  private validateIcons(): void {
    if (!this.manifest.icons) {
      this.addError('warning', 'Icons are recommended for better user experience', 'icons')
      return
    }
    ...
    for (const size of recommendedSizes) {
      if (!this.manifest.icons[size]) {
        this.addError('warning', `Icon size ${size}x${size} is recommended`, `icons.${size}`)
      } else if (!this.fileExists(this.manifest.icons[size])) {
        this.addError('error', `Icon file not found: ${this.manifest.icons[size]}`, `icons.${size}`)
      }
    }
  }
  ...
  private validateBackground(): void {
    if (!this.manifest.background) {
      this.addError('info', 'No background service worker defined', 'background')
      return
    }
  ...
  private validatePermissions(): void {
    ...
    if (permissions.length > 5 && !this.manifest.optional_permissions?.length) {
      this.addError(
        'info',
  ...
  private validateCSP(): void {
    ...
    if (csp?.extension_pages) {
      ...
      const unsafeDirectives = ['unsafe-eval', 'unsafe-inline', 'unsafe-hashes']
