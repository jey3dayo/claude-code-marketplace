...
class PermissionAnalyzer {
  ...
  analyze(): AnalysisResult {
    ...
    const unusedPermissions = declaredPermissions.filter(perm => !requiredPermissions.has(perm))
  ...
  private generateSuggestions(data: {
    ...
    if (data.unusedPermissions.length > 0) {
      suggestions.push(`Remove unused permissions: ${data.unusedPermissions.join(', ')}`)
    }

    // Missing permissions
    if (data.missingPermissions.length > 0) {
      suggestions.push(`Add missing permissions: ${data.missingPermissions.join(', ')}`)
    }

    // Overly broad host permissions
    for (const hostPerm of data.declaredHostPermissions) {
      if (hostPerm === '*://*/*' || hostPerm === '<all_urls>') {
        suggestions.push(`Consider narrowing host permission: "${hostPerm}" → specific domains`)
      }
    }

    // Optional permissions suggestion
    const nonCritical = ['tabs', 'bookmarks', 'history']
    const canBeOptional = data.declaredPermissions.filter(p => nonCritical.includes(p))
    if (canBeOptional.length > 0) {
      suggestions.push(`Consider making these permissions optional: ${canBeOptional.join(', ')}`)
    }
