...
function isRecord<T>(
  ...
  return typeof data === 'object' && data !== null && Object.values(data).every(valueGuard)
}
```
...
```typescript
// 統一された日付型
export const DateOrString = z.union([z.string().datetime(), z.instanceof(Date)])
...
const createUserWorkflow = (input: UserCreateInput) =>
  validateUserInput(input)
    .andThen(validatedData => checkEmailUniqueness(validatedData.email).map(() => validatedData))
...
export class UserService {
  ...
  async getUser(id: string): Promise<Result<User, ServiceError>> {
    return handleApiResponse(this.api.get(`/users/${id}`), UserResponseSchema).map(
      response => response.data
    )
  }

  // 複数データソースの統合
  async getUserWithDetails(id: string): Promise<Result<UserWithDetails, ServiceError>> {
...
export async function createUserAction(
  ...
  // FormDataから値を抽出
  const extractedData = extractFormFields(formData, ['name', 'email', 'department'])
...
// 使用例
const prismaToUser = createSafeTransformer(PrismaUserSchema, UserSchema, prismaUser => ({
  id: prismaUser.id,
